...
- Većina funkcija radi u 2D Euklidovom prostoru (metrički sistem proizvoljan)
- Centerline se tretira kao polilinija (niz segmenata između susednih tačaka).
"""
...
class Track:
    """
    Jednostavan tip podatka koji objedinjava srednju liniju i širinu.
...
def make_s_track(n: int = 350, scale: float = 1.0) -> Track:
    ...
    - n: broj uzoraka duž centerline (veći n -> glađa linija)
    ...
    """
    xs = np.linspace(0, 120*scale, n) # pravimo n ravnomerno raspoređenih tačaka - x koordinate staze
    ys = 8*scale*np.sin(xs/(12*scale)) + 3*scale*np.sin(xs/(4*scale)) # amplitude i periode utiču na "zakrivljenost" sinusa
    center = list(zip(xs.tolist(), ys.tolist())) # ovo je centerline polilinija
...
def make_random_track(n: int = 420,
    ...
    Parametri:
      ...
      - jitter: maksimalna devijacija od radijusa (ili y-odstupanje za otvorene staze)
    ...
    if closed:
        ...
        #endpoint = False znači da ne uključujemo poslednju vrednost iz opsega
...
def polyline_segments(points: List[Point]):
    ...
    - Yield: (p0, p1), (p1, p2), ...
    Koristi se za pretragu najbližih segmenata i za izračunavanje Cross-Track Error
    (udaljenosti vozila od srednje linije staze).
    """
...
def closest_point_on_segment(p: Point, a: Point, b: Point) -> Tuple[Point, float]:
    ...
    vv = vx*vx + vy*vy  # |AB|^2
    if vv == 0:
        # ako segment nema duzinu, najbliza tacka je A
        return (a, 0.0)
    # parametar t = proj(AP) na AB / |AB|^2
    t = (wx*vx + wy*vy) / vv
    # klipujemo na segment
    t = max(0.0, min(1.0, t)) # to znaci da cemo, ako proj padne van segmenta, uzeti najblizi kraj
    cx, cy = ax + t*vx, ay + t*vy   # C = A+ t*AB
    return ( (cx, cy), t )
...
def distance_to_centerline(p: Point, centerline: List[Point]) -> Tuple[float, int, Point]:
    ...
    Vraća:
      ...
      - best_i: indeks segmenta (a,b) na kojem je najbliža tačka (vraća i takav
      da segment=(centerline[i], centerline[i+1]))
    ...
    Napomene:
      - F-ja ne radi ako ima manje od 2 tacke, ali podrazumeva se da se nece zadati takav slucaj.
    """
    best_d2 = float("inf")
    best_i = 0
...
def find_lookahead_goal(p: Point, centerline: List[Point], start_seg_idx: int, Ld: float) -> Point:
    ...
    p                # trenutna pozicija vozila (ovde se ne koristi direktno)
    centerline       # lista tačaka staze
    start_seg_idx    # indeks segmenta gde je vozilo trenutno najbliže
    Ld               # lookahead distance (koliko unapred gledamo, odredjuje preciznost)

    Algoritam:
      - kreće od tačke centerline[start_seg_idx] i prati segmente sve dok
      ...
      - vraća poslednju posećenu centerline tačku (aproksimacija: ne interpolira tačno
        na segmentu ako segment prelazi Ld; za veću preciznost se može dodati linearna interpolacija po preostalom delu segmenta).
    ...
    """
    idx = max(0, min(len(centerline)-2, start_seg_idx)) # osiguravamo da je indeks validan
    dist = 0.0 # koliko smo presli duz staze do sada
    cur = centerline[idx]
    i = idx
...
# Hermitska interpolacija: krajnje tacke segmenta i njihove tangente (nagibi)
def _catmull_rom_one_segment(p0, p1, p2, p3, n_points):
    """
    Izračunava n_points segmenata Catmull–Rom glatke krive između p1 i p2 koristeći p0 i p3
    ...
    - endpoint=False u linspace-u znači da ne uključujemo tačku p2
    (to se rešava u glavnoj petlji gde se segmente spajaju).
    """
    ...
    # spline prvo odredjuje za x, pa za y koordinate
...
def catmull_rom_chain(points, n_points=400, closed=True):
    """
    Izračunava Catmull-Rom interpoliranu krivu kroz zadate kontrolne tačke.
    Parametri:
    ...
    Edge-cases i napomene:
      ...
      - Komplikovanije varijante bi koristile adaptivno uzorkovanje po dužini segmenta, ali
        za naše svrhe uniformna podela po segmentima je jednostavna i dovoljna.
    """
    if len(points) < 2:
        return points[:]
    ...
    if closed:
        ...
        # za otvorenu krivu: dupliraj krajnje kontrolne tačke
